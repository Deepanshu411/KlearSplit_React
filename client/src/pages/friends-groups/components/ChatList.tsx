...
const ChatList: React.FC<ChatListProps> = ({
  ...
  const [currentChat, setCurrentChat] = useState<FriendData | GroupData | null>(
    null
  );

  const handleCancel = () => {
    setOpenConfirm(false);
    setCurrentChat(null); // Reset the current chat when dialog is closed
  };

  const onReject = (chat: FriendData | GroupData) => {
    setCurrentChat(chat); // Store the chat to be rejected
    setOpenConfirm(true); // Open the confirmation dialog
  };

  const handleConfirm = async () => {
    if (currentChat) {
      await handleAcceptReject(currentChat, "REJECTED");
      setOpenConfirm(false); // Close the dialog after rejection
    ...
    <>
      <ConfirmDialog
        ...
        description={`Are you sure you want to reject this ${
          isFriendsConversations(chats) ? "request" : "invite"
        }?`}
                ...
                {friend.isRequest && friend.status === "RECEIVER" && (
                  <div className="flex flex-row items-center gap-2">
                    <Tooltip title="Accept Request" placement="top">
                      <IconButton
                        color="success"
                        onClick={() => handleAcceptReject(friend, "ACCEPTED")}
                      >
                        <Check />
                      </IconButton>
                    </Tooltip>
                ...
                {group.status === "PENDING" && (
                  <div className="flex flex-row items-center gap-2">
                    <Tooltip title="Accept Invite" placement="top">
                      <IconButton
                        color="success"
                        onClick={() => handleAcceptReject(group, "ACCEPTED")}
                      >
                        <Check />
                      </IconButton>
                    </Tooltip>
