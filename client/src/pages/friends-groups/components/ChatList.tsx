      ...
      {friends.map((friend, index) => (
        <div key={index} className="flex flex-row items-center justify-between w-full p-4 cursor-pointer active:cursor-grabbing" onClick={() => onSelectFriend(friend)}>
          <div className="flex flex-row items-center gap-4 w-full">
            <img src={friend.friend.image_url || "https://randomuser.me/api/portraits/men/9.jpg"} alt={friend.friend.first_name} className="h-6 w-6 xl:h-10 xl:w-10 rounded-full" />
            <div className="flex flex-col">
              <h5 className="xl:text-md text-sm font-semibold">{friend.friend.first_name} {friend.friend.last_name}</h5>
              <p className="text-xs xl:text-sm">{friend.friend.email}</p>
            </div>
          </div>
          <h6 className={`text-sm xl:text-md ${getBalanceAsNumber(friend.balance_amount) < 0 ? "text-red-600" : "text-green-600"}`}>
